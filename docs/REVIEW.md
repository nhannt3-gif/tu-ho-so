# REVIEW — App "Tủ hồ sơ" (bản 3.29, cập nhật 26/09/2026 18:19)

- **Phạm vi rà soát:** toàn bộ `index.html` (11.670 dòng — CSS, HTML, JavaScript) và `README.md`. Phần CSS (dòng 7–1211) chỉ đọc lướt, không rà từng quy tắc.
...
- Repo `nhannt3-gif/tu-ho-so` đang ở chế độ **public**, và GitHub Pages cũng công khai. Dữ liệu nằm trong `MAC_DINH.diaBan` (`index.html:1484–1578`).
...
- Từ khi bỏ mã PIN, khóa AES được tạo ngẫu nhiên rồi **lưu dạng thô ngay trong `localStorage`** (`D.cauHinh.hsKhoa`, `index.html:7843–7844`), cạnh ảnh đã mã hóa trong IndexedDB. Ai mở được trình duyệt trên máy đó, hoặc có phần mềm đọc được dữ liệu trình duyệt, đều giải mã được.
- Giao diện vẫn ghi "Ảnh trong máy vẫn mã hóa" (`index.html:6718`, 6799) nên dễ tạo cảm giác an toàn chưa đúng.
- Nếu `crypto.subtle` không dùng được, ảnh được lưu **không mã hóa** mà không báo rõ (`index.html:7973`).
...
- `goiChiMuc()` (`index.html:10950–10961`) đưa `scan` (họ tên khách, xã, ấp, tổ, ghi chú) vào `chimuc.json` và 7 bản `du_phong/chimuc_*.json` trên Drive. Ảnh thì không đưa lên, nhưng danh sách khách thì có.
...
- Danh mục có các loại "Sao kê khách hàng", "Sao kê tín dụng chi tiết" (`index.html:1596–1597`). Nút 📋 chép nguyên bảng để dán vào AI mà không cảnh báo về dữ liệu cá nhân (khác với tab Scan, nơi app có cảnh báo).

**🟡 R5. Thư viện ngoài được nạp và cất vĩnh viễn, không kiểm tra toàn vẹn, không bao giờ cập nhật.** [Đọc mã — chắc chắn]
- `napMotTV` (`index.html:1420–1440`) tải pdf.js, pdf-lib, SheetJS từ cdnjs rồi chèn vào trang, **không có SRI (mã kiểm tra toàn vẹn)**. Bản đã cất vào IndexedDB được dùng mãi (khóa theo tên `pdfjs`, `xlsx`… chứ không theo URL/phiên bản). Nếu sau này đổi phiên bản trong `THU_VIEN`, máy cũ vẫn chạy bản cũ.
...
- Nhiều chỗ ghép `id`, tên xã, mã báo cáo thẳng vào chuỗi `onclick="...('...')"` (ví dụ `index.html:4120`, 4270, 9197). Nếu `chimuc.json` / `cauhinh.json` / `nguon-cau.json` trên Drive bị người khác sửa (khi thư mục được chia sẻ), có thể chèn mã chạy trong app — mà app đang giữ token Drive. Hiện tại chỉ anh ghi các file này nên rủi ro thấp.

### 2.2. Lỗi chức năng

**🔴 L1. Cài đặt → Địa bàn bị hỏng hoàn toàn (lỗi `demDiaBan is not defined`).** [Đã chạy thử]
- Hàm `demDiaBan()` **không được khai báo ở đâu cả**, trong khi 7 chỗ gọi tới nó (`index.html:6565`, 11152, 11168, 11373, 11375, 11465, 11532). Hàm `demDiaBan2()` lại gọi chính `demDiaBan`. Có thể hàm này bị mất khi ghép file.
- Hậu quả:
...
- `luuCDTab` (`index.html:6765–6775`) dựng lại `mauBaoCao` từ ô chữ và chỉ giữ `ten, ma, tuKhoa, cap`. Các cờ `theoNgay`, `gopPGD`, `coExcel`, `thuanXLS`, `chuKy`, `an` **bị xóa**.
...
- **Nạp Excel** (`index.html:11485–11496`) cũng có cùng lỗi với sheet "Mau bao cao" (còn làm mất cả `cap`).

**🟠 L3. Thả file vào ô "kéo thả" ở màn Thêm file thì file bị xử lý 2 lần.** [Đã chạy thử]
- Ô `#vung-tha` có trình xử lý `drop` riêng (`index.html:11607–11610`), sự kiện lại nổi lên `document` và gặp trình xử lý thứ hai (`index.html:11631–11634`). Cả hai cùng gọi `gioiThieuFile`.
...
- `dongGoThem(el, iso)` (`index.html:2549`) đọc biến toàn cục `event` và chỉ chạy khi `event.key==='Enter'`. Khi bấm chuột, `event` là sự kiện click nên hàm thoát ngay (tham số `{key:'Enter'}` truyền vào ở `index.html:3271` bị bỏ qua). Gõ Enter bằng bàn phím vẫn chạy bình thường.

**🟡 L5. Nhấn Enter ở giữa danh sách TO-DO không thêm được dòng mới.** [Đã chạy thử]
- `dongPhim` gọi `dongThem(iso, ' ')` (`index.html:2697`), nhưng `dongThem` từ chối chuỗi chỉ có dấu cách (`index.html:2542`) nên trả về `null`.

**🟡 L6. "Nhờ AI chuẩn hóa" ghi cột `nghiep_vu` vào **Tag** chứ không vào **Mảng nghiệp vụ**.** [Đọc mã — chắc chắn]
- Từ bản 3.4, mảng là `m.mang` (chọn một), tag là `m.the`. Nhưng `xuatChoAI` xuất `m.the` ra cột `nghiep_vu`, và `napTuAI` (`index.html:10641–10642`) ghi kết quả vào `m.the`. Câu nhắc lại bảo AI chọn trong danh sách **mảng**. Kết quả: danh sách tag bị lẫn tên mảng, mảng vẫn trống.
...
- Nút "Đóng" trong hộp Nhờ AI gọi `moCaiDat('cm')` (`index.html:10542`), mục `cm` không tồn tại → Cài đặt mở ra trang trống. [Đã chạy thử]

**🟡 L7. Dán kết quả AI: ghép mục theo số hiệu dễ ghép nhầm văn bản.** [Đọc mã]
- `ghepMuc` (`index.html:10754–10762`) so **chỉ phần số** trước dấu "/" (ví dụ "958"), và mặc định chọn "Lấy AI". Hai văn bản khác cơ quan cùng số 958 sẽ bị ghép nhầm, người dùng bấm "Áp dụng" là ghi đè sai.
...
- `taiChiMucTuDrive` bỏ qua việc tải khi `modifiedTime` (giờ máy chủ Google) ≤ `chiMucThoi` (giờ **của máy tính**) (`index.html:11066`). Đồng hồ máy chạy nhanh là có thể bỏ lỡ bản mới hơn.
...
- Xóa scan (`xoaScanIm`) không ghi dấu đã xóa, còn `gopNhanh` (`index.html:11049–11058`) thêm mọi mục có trên Drive mà máy này chưa có. Máy khác chưa xóa, hoặc bản `chimuc.json` cũ, sẽ đưa mục đã xóa quay lại (ảnh thì đã mất).

**🟡 L10. Khôi phục từ file dự phòng chỉ lấy lại một phần.** [Đọc mã — chắc chắn]
- `xuatDuPhong` gọi là "bản dự phòng đầy đủ" (có Biểu mẫu, Scan, Lịch/ghi chép, Thùng rác…), nhưng `napDL` (`index.html:6991–7013`) chỉ nạp lại `vanBan`, `duLieu`, `ghiChu` và cài đặt. **Lịch, sổ ghi chép, biểu mẫu, scan, thùng rác không khôi phục được** từ file này. Điều này quan trọng vì "Xóa sạch máy" dựa vào chính file dự phòng này.

**🟡 L11. Quét Drive bỏ sót file khi thư mục có trên 200 mục.** [Đọc mã — chắc chắn]
- `gomFile` (`index.html:7501–7522`) và `napKhayCho` (`index.html:7335`) gọi API với `pageSize=200` nhưng **không đọc trang tiếp** (`nextPageToken`). Thư mục nào có trên 200 file thì phần dư bị bỏ qua mà không báo. (Hàm quét dọn `gomTatCaDon` thì đã xử lý phân trang đúng.)

**🟡 L12. Khay `_Chờ xử lý` không thấy file anh tự bỏ vào bằng Drive/ổ G.** [Đọc mã — theo tài liệu quyền `drive.file` của Google]
- Với quyền `drive.file`, app chỉ thấy file **do app tạo**. Chính app đã ghi điều này (`index.html:7706`, 9081). Nhưng màn Thêm file và cây thư mục vẫn hướng dẫn "lấy file anh đã bỏ vào thư mục _Chờ xử lý trên Drive" (`index.html:4705`, 11200) → hướng dẫn mâu thuẫn, dễ tưởng app lỗi.
...
- `Promise.all(ds).then(...)` (`index.html:1859`) không có nhánh bắt lỗi. Nếu một trang không đọc được chữ, Promise không bao giờ kết thúc → file đó kẹt ở "Đang đọc file…", thanh tiến độ không chạy xong.

**⚪ L14. Các lỗi nhỏ khác** [Đọc mã]
- Chấm trên lịch khi gõ ghi chép không cập nhật ngay: `capNhatChamNgay` tìm `.lc-o[data-iso]` nhưng ô lịch không có thuộc tính `data-iso` (`index.html:2757`). [Đã chạy thử: 0 ô]
- `lcChonNgay` gọi `nkLuu()` không tồn tại; hiện không lỗi vì `NK_HEN` không bao giờ được gán (mã thừa, `index.html:3275`, 3305).
- "Duyệt tất cả" với file lấy từ khay: đổi tên trên Drive **2 lần** cho mỗi file (trong `duyetThat` và trong `lamDuyetHet`, `index.html:4959`, 4993–5001).
- Lịch âm: ngày 29 tháng Chạp luôn ghi "Giao thừa" kể cả tháng đủ 30 ngày (`index.html:2394`).
- Chọn lẫn bản "Thẻ" và bản "Tài liệu" rồi In/Lưu: chỉ xử lý các bản Tài liệu, bản Thẻ bị bỏ qua không báo (`index.html:8350–8352`).
- `luuKhach`, `luuScanTL` tự điền xã/ấp/tổ **của lần trước** khi bỏ trống (`index.html:8214–8217`) → dễ gắn sai địa bàn cho khách.
- `boiCanh` ghi cứng "PGD Gò Dầu" thay vì lấy tên đơn vị trong Cài đặt (`index.html:4416`).
...
- Nội dung "kiến thức": "Việt Nam có 63 tỉnh, thành phố" (`index.html:2939`) đã lỗi thời sau sắp xếp đơn vị hành chính năm 2025.
- Hướng dẫn ghi "File gốc trên Drive không bao giờ bị đụng tới" (`index.html:6880`) — không đúng nữa, vì app có đổi tên, dời thư mục và **xóa vĩnh viễn** file trên Drive.

### 2.3. Rủi ro vận hành

**🟠 V1. "Xóa hẳn" xóa vĩnh viễn trên Drive, bỏ qua thùng rác của Google.** [Đọc mã]
- `xoaFileDrive` dùng lệnh `DELETE` (`index.html:6275–6284`). Không có lớp an toàn 30 ngày của Google. Dấu `daXoaHan` còn đồng bộ sang máy khác, và "Dọn dữ liệu thử" có ô "Xóa hẳn luôn".
...
**⚪ V4. Khả năng bảo trì.**
- 11.670 dòng trong một file, 25 khối `<script>` (có 5 khối rỗng), nhiều hàm trùng chức năng (`goNgay`/`gonNgay`, `ngayISOo`/`ngayISOtu`/`ngayVao`…), mã chết (`if(false){…}` ở `index.html:3749`, `moHoSoCu`, `datPIN`, `kiemPIN`). Mỗi bản sửa rất dễ làm hỏng chỗ khác (L1 là ví dụ).
